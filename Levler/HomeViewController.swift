...
//    @IBOutlet weak var overall_outoflocation: UILabel!
 ...
 //   @IBOutlet weak var total_outoflocation: UILabel!
    ...
    //Out of Locations Label
 /*   func outOfLocations(){
        let defaultValue = UserDefaults.standard
        let token = defaultValue.string(forKey: "token")
        let Auth_header    = ["Authorization" : token]
        
        request("https://app.levler.co/api/v1/total/locations", method: .get, parameters: nil , encoding: JSONEncoding.default, headers: (Auth_header as? HTTPHeaders)).validate().responseJSON { response in
            print("No of locations: \(response.result)")
            
            switch response.result {
            case .success:
                let json = response.result.value as! NSDictionary
                let loc = json["data"] as! Int
                self.overall_outoflocation.text = "Out of \(loc) locations."
                self.total_outoflocation.text = "Out of \(loc) locations."
            case .failure:
                print("error")
            }
        }
 ...
 */
    ...
    //PROGRESS BAR
/*    func  progressBarDetails()
    {
        let defaultValues = UserDefaults.standard
        
        let team_goal = defaultValues.string(forKey: "team_goal")
       // let total_review = defaultValues.string(forKey: "totalreview")
        let totalprogress = Double(team_goal!)
        goal_totalgoalLabel.text =  String(Int32(totalprogress!))
        // let my_progress = defaultValues.string(forKey: "invites_sent_count")
        let myprogress: Double? = Double(totalreview!)
        if (myprogress!)>=(totalprogress)!
        {
            goal_description.text = "You have achieved your goal"
            goal_finalLabel.text = "😍"
            progress_tracklabel.text = "You're on track!"
            progress_tracklabel.textColor = UIColor(red: 96/255, green: 189/255, blue: 106/255, alpha: 1.0)
            
            progress_catchuplabel.text = " Enjoy your achievement 🎖"
        }
        else
        {
            let recmd = Int(totalprogress! - myprogress!)
            goal_finalLabel.text = "\(recmd) invites"
            progress_catchuplabel.text = " Send \(recmd*2) invites to catchup"
        }
        let progress: Double = Double(myprogress!)/Double(totalprogress!)
        print("PROGRESS % \(progress)")
        
        myProgress.text = String(Int(myprogress!))
        totalProgress.text = String(Int32(totalprogress!))
  ...
  func allReviews()
  {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
       ...
       // print(rateString)
       /* request(String(describing: reviews.imageURL), method: .get).response { response in
            cell.reviewimage.image = UIImage(data: response.data!, scale:1)
        }*/
        dataExist()
       ...
       /* if a >= 45
        {
            x = 160
            cell.readless.isHidden = true
            cell.readmore.isHidden = false
        }
        if a >= 45 && cell.readmore.isSelected
        {
            x = 120 + a
            cell.readmore.isHidden = true
            cell.readless.isHidden = false
        }
        else if a >= 45  && cell.readless.isSelected
        {
            x = 160
            cell.readmore.isHidden = false
            cell.readless.isHidden = true
